...
##$JREPath = "C:\Program Files\\Microsoft\jdk-$OpenJDKVersion-hotspot" ## Note that if you're running 32bit java, you will need to change this path
...
if($solrSSL -eq $true)
{
    ...
    if(!(Test-Path -Path "$solrRoot\bin\solr.in.cmd.old"))
    {
        ...
        $cfg = Get-Content "$solrRoot\bin\solr.in.cmd"
        Rename-Item "$solrRoot\bin\solr.in.cmd" "$solrRoot\bin\solr.in.cmd.old"
        $newCfg = $cfg | % { $_ -replace "REM set SOLR_SSL_KEY_STORE=etc/solr-ssl.keystore.p12", "set SOLR_SSL_KEY_STORE=$certStore" }
        $newCfg = $newCfg | % { $_ -replace "REM set SOLR_SSL_KEY_STORE_PASSWORD=secret", "set SOLR_SSL_KEY_STORE_PASSWORD=secret" }
        $newCfg = $newCfg | % { $_ -replace "REM set SOLR_SSL_TRUST_STORE=etc/solr-ssl.keystore.p12", "set SOLR_SSL_TRUST_STORE=$certStore" }
